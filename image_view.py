...
imaDialation = cv2.dilate(imgCanny,karnel,iterations=5)
imaErode = cv2.erode(imaDialation,karnel,iterations=5)

# iamge view
cv2.imshow("Messi Output",imgGray)
cv2.imshow("Messi Blur",imageBlur)
cv2.imshow("Messi canny",imgCanny)
cv2.imshow("Messi dialation",imaDialation)
cv2.imshow("Messi imaErode",imaErode)
cv2.waitKey(0)
